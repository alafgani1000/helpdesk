<form id="form-attach-user" action="{{ route('role.assign', $user->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
...
<script>
    $(function(){
        ...
        table_role = $("#dt_role").DataTable({
                ...
                serverSide:true,
                ajax:{
                    url:"{{ url('role/data/role/') }}/"+user_id,
                    type:"GET",
                    dataType:"JSON",
                    data:{
                        "_token":"{{csrf_token()}}"
        ...
        $("#form-attach-user").submit(function(event){
            ...
            .fail(function(data){
                if(data.responseJSON.errors.role_id){
                    $("#help-role").html(data.responseJSON.errors.role_id);
                }
        ...
        $("body").on('click','.deletedrole',function(event){
            ...
            let url = "{{ url('role/dettach/') }}/"+user_id;
            if(confirm('Delete ?')){
                $.ajax({
                url:url,
                type:"POST",
